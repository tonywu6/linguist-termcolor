...
//! color differences using [`color_art::distance_with`][distance_with].
//!
...
//! See [`ColorSpace`][ColorSpace] for available choices.
//!
...
struct Linguist(HashMap<String, LinguistLang>);
...
impl Linguist {
    ...
    fn colors(&self) -> anyhow::Result<ColorMap<'_>> {
        let colors = self
            .0
            .values()
            .map(|lang| {
...
struct ColorMap<'a>(HashMap<Cow<'a, str>, Vec<(Cow<'a, str>, u32)>>);

impl ColorMap<'_> {
    fn query(&self, query: &str) -> BTreeMap<Cow<'_, str>, TermColor> {
        tokenize(query)
            .iter()
            .copied()
...
impl TermColor {
    fn print(&self, colors: ColorSpace) -> String {
        let color = self.0;
